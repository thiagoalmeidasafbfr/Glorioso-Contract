  ...
  return (
    <div>
      <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 10, padding: '16px 20px' }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: labelColor, textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: font }}>{label}</div>
        <div style={{ fontSize: 24, fontWeight: 800, color: valueColor, marginTop: 8, fontFamily: font, lineHeight: 1.1 }}>{value}</div>
        {sub && <div style={{ fontSize: 10, color: labelColor, fontFamily: font, marginTop: 4, opacity: 0.8 }}>{sub}</div>}
      </div>
      {footnote && (
        <div style={{ fontSize: 10, color: '#aaa', fontStyle: 'italic', fontFamily: font, marginTop: 4, paddingLeft: 2 }}>
          {footnote}
        </div>
      )}
    </div>
...
export default function PageIntermediarios() {
  ...
  const th: React.CSSProperties = {
    ...
    color: '#555', background: '#f8f8f8', borderBottom: '2px solid #e8e8e8',
    fontFamily: font, whiteSpace: 'nowrap', position: 'sticky', top: 0, zIndex: 1,
    overflow: 'hidden', textOverflow: 'ellipsis',
    cursor: 'pointer', userSelect: 'none',
  }
  const td: React.CSSProperties = {
    padding: '14px 10px', fontSize: 12, color: '#333', fontFamily: font,
    ...
    <div style={{ padding: '12px 16px', maxWidth: 1600, margin: '0 auto', fontFamily: font }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr 200px', gap: 12, marginBottom: 12, alignItems: 'stretch' }}>
        ...
        <div className="card" style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <div style={{ fontSize: 9, fontWeight: 700, color: '#999', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4, fontFamily: font }}>{t('Intermediário')}</div>
          ...
          <div>
            <div style={{ fontSize: 9, fontWeight: 700, color: '#999', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4, fontFamily: font }}>{t('Atleta')}</div>
          ...
          <div>
            <div style={{ fontSize: 9, fontWeight: 700, color: '#999', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4, fontFamily: font }}>{t('Certo/Condicional')}</div>
          ...
          <div style={{ marginTop: 'auto', color: '#bbb', fontSize: 11, fontFamily: font }}>
        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 32px', gap: 20 }}>
          {interNome ? (
            <>
              ...
              <div>
                <div style={{ fontSize: 11, color: '#bbb', fontFamily: font, marginBottom: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Intermediário')}</div>
                <div style={{ fontSize: 26, fontWeight: 800, color: '#111', fontFamily: font, lineHeight: 1.1 }}>{interNome}</div>
                <div style={{ fontSize: 11, color: '#aaa', fontFamily: font, marginTop: 6 }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: '#ccc', fontFamily: font, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Intermediário')}</div>
              <div style={{ fontSize: 20, fontWeight: 600, color: '#bbb', fontFamily: font }}>—</div>
              <div style={{ fontSize: 11, color: '#ccc', fontFamily: font, marginTop: 4 }}>{t('Selecione um intermediário para exibir')}</div>
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', justifyContent: 'center', borderTop: '4px solid #111' }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: '#999', textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: font }}>{t('Valor Total')} ({symbol})</div>
          <div style={{ fontSize: 30, fontWeight: 800, color: '#111', marginTop: 10, fontFamily: font, lineHeight: 1 }}>{fmtMiC(totalGeral)}</div>
          <div style={{ fontSize: 10, color: '#bbb', fontFamily: font, marginTop: 6 }}>{t('Saldo total filtrado')}</div>
        </div>
      </div>
          ...
          <table style={{ tableLayout: 'auto', width: '100%' }}>
            ...
            <tbody>
              ...
              {sorted.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ background: p.status === 'Atrasado' ? '#fff5f5' : undefined }}>
