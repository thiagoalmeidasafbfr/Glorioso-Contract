...
export default function PageImagem() {
  ...
  const td: React.CSSProperties = {
    padding: '12px 10px', fontSize: 12, color: '#333', fontFamily: font,
  ...
  return (
    <div style={{ padding: '12px 16px', maxWidth: 1600, margin: '0 auto', fontFamily: font }}>
      ...
      <div className="card" style={{ padding: '12px 16px', marginBottom: 12, display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        <div style={{ marginLeft: 'auto', color: '#bbb', fontSize: 11, fontFamily: font }}>
      ...
      <div className="card" style={{ padding: '16px', marginBottom: 12 }}>
        <div style={{ fontWeight: 700, fontSize: 14, color: '#111', marginBottom: 12, fontFamily: font }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 12 }}>
          {filtrados.map(a => (
            <div key={a.id} style={{
              ...
              border: '1px solid #e8e8e8', transition: 'transform 0.15s, box-shadow 0.15s',
              ...
              <div style={{ padding: '8px', textAlign: 'center', flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#333', lineHeight: 1.3 }}>{a.nome}</div>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#000' }}>{fmtMiC(a.direitoImagem)}</div>
                <button
                  onClick={() => navigateToAtleta(a.id)}
                  style={{
                    marginTop: 4, padding: '4px 0', fontSize: 10, fontWeight: 600,
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {atrasadas > 0 ? (
                          <button onClick={() => toggleExpand(a.id)} style={{
                            ...
                            fontSize: 10, color: '#c0392b', fontWeight: 700, lineHeight: 1, flexShrink: 0,
                          }}>
                            {expanded ? '▼' : '▶'}
                          </button>
                        ) : (
