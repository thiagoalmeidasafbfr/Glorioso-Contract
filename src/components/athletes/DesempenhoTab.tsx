...
export default function DesempenhoTab({ athleteId, onChanged }: { athleteId: string; onChanged?: () => void }) {
  ...
  async function remove(r: Desempenho) {
    if (!await confirm({ title: `Excluir o desempenho ${r.temporada}${r.competicao ? ` · ${r.competicao}` : ' (total)'}?`, message: 'Esta ação não pode ser desfeita.', danger: true })) return
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {canWrite ? (
        <div className="card" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 11, fontFamily: mono, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-secondary)', fontWeight: 700 }}>Lançar / atualizar desempenho</div>
