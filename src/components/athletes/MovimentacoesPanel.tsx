...
export default function MovimentacoesPanel({ athleteId, refreshKey, onChanged }: {
  ...
  async function undo(m: Movimentacao) {
    if (!window.confirm(`Desfazer a movimentação "${MOVIMENTACAO_LABELS[m.tipo]}" de ${fmtDate(m.data_movimentacao)}? O status do atleta, os vínculos, as parcelas canceladas e a titularidade voltam ao estado anterior.`)) return
    setBusy(m.id)
    try {
      const r = await desfazerMovimentacao(m.id)
      await load()
      onChanged()
      window.alert(`Movimentação desfeita. Parcelas restauradas: ${r.parcelas_restauradas}; direitos de imagem restaurados: ${r.direitos_imagem_restaurados}.`)
