...
export default function LoanShareModal({
  ...
  const cell = (label: string, value: string, hi?: boolean) => (
    <div style={{ padding: '10px 14px', borderRadius: 9, background: hi ? 'var(--pos-tint)' : 'var(--bg-subtle)', border: `1px solid ${hi ? 'rgba(47,107,58,0.25)' : 'var(--divider)'}` }}>
      <div style={{ fontSize: 11, fontFamily: mono, letterSpacing: '0.14em', textTransform: 'uppercase', color: hi ? 'var(--pos)' : 'var(--text-muted)', marginBottom: 5 }}>{label}</div>
    ...
    <ModalShell title="Rateio de salário no empréstimo" width={640} onClose={onClose}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: mono, marginTop: 4 }}>
      ...
      {existing.length > 0 && (
        <div style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: font, padding: '9px 12px', borderRadius: 8, background: 'var(--bg-subtle)', border: '1px solid var(--divider)' }}>
