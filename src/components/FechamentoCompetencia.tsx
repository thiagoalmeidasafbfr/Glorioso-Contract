...
export default function FechamentoCompetencia({ compute, names, disabled }: {
  ...
  async function close() {
    ...
    if (!await confirm({ title: `${already ? 'Re-fechar' : 'Fechar'} a competência ${fmtComp(c)}?`, message: `${rows.length} atleta(s). Amortização do mês: ${fmtCurrencyShort(total, 'BRL')}.${already ? ' Os valores gravados anteriormente serão substituídos.' : ''}`, confirmLabel: already ? 'Re-fechar' : 'Fechar competência' })) return
  ...
  const th: React.CSSProperties = { padding: '7px 10px', fontSize: 11, fontFamily: mono, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-muted)', textAlign: 'left', borderBottom: '1px solid var(--divider-soft)', fontWeight: 600 }
    ...
    <div className="card" style={{ padding: '16px 18px', marginTop: 16 }} data-testid="fechamento-competencia">
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 220 }}>
          <div style={{ fontFamily: mono, fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--gold-deep)' }}>Fechamento por competência</div>
        ...
        <div>
          <label htmlFor="fech-comp" style={{ fontSize: 11, fontFamily: mono, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-muted)', display: 'block', marginBottom: 3 }}>Competência</label>
