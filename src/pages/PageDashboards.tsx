...
import {
  fmtCurrencyShort, fmtCurrencyParts, isOverdue, todayISO, daysFromToday, monthShort,
} from '../lib/format'
...
const monthLabel = (ym: string) => { const [y, m] = ym.split('-'); return `${monthShort(+m)}/${y.slice(2)}` }
...
export default function PageDashboards() {
  ...
  const aging = useMemo(() => {
    ...
    const buckets: { name: string; min: number; max: number; total: number; byNat: Record<string, number>; color: string }[] = [
      { name: tr('0-30 dias'),    min: 1,   max: 30,    total: 0, byNat: {}, color: AGING_COLORS[0] },
      { name: tr('31-60 dias'),   min: 31,  max: 60,    total: 0, byNat: {}, color: AGING_COLORS[1] },
      { name: tr('61-90 dias'),   min: 61,  max: 90,    total: 0, byNat: {}, color: AGING_COLORS[2] },
      { name: tr('91-180 dias'),  min: 91,  max: 180,   total: 0, byNat: {}, color: AGING_COLORS[3] },
      { name: tr('Acima de 180'), min: 181, max: 99999, total: 0, byNat: {}, color: AGING_COLORS[4] },
    ]
    for (const i of items) {
  ...
  return (
    <div style={{ padding: '24px 28px 32px', width: '100%', boxSizing: 'border-box' }}>
      <PageHero title={tr('Painel executivo')} subtitle={tr('Controle financeiro · Botafogo SAF')} />

      {loading ? (
        <div className="eyebrow" style={{ padding: '40px 4px' }}>{tr('Carregando…')}</div>
      ) : (
        <>
              ...
              return (
                <div key={t.label} className="stat-tile" style={{ padding: 'var(--gutter-card)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span className="eyebrow">{tr(t.label)}</span>
                  </div>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, lineHeight: 'var(--text-metric-line)', whiteSpace: 'nowrap' }}>
                      <span style={{ fontSize: 'var(--text-subtitle-size)', fontWeight: 400, color: 'var(--text-muted)' }}>{tr(p.sym)}</span>
                      <span style={{ fontSize: 'var(--text-metric-size)', fontWeight: 400, letterSpacing: 'var(--text-metric-tracking)', fontVariantNumeric: 'tabular-nums', color: alarm ? 'var(--text-negative)' : 'var(--text-primary)' }}>{tr(p.num)}</span>
                      {p.suffix && <span style={{ fontSize: 'var(--text-title-size)', fontWeight: 400, color: 'var(--text-muted)' }}>{tr(p.suffix)}</span>}
                    </div>
                    <div style={{ marginTop: 2, fontSize: 'var(--ui-text-size)', color: 'var(--text-secondary)' }}>{tr(t.sub)}</div>
                  </div>
                </div>
              )
            })}
          ...
          <Panel eyebrow={tr('Panorama')} title={tr('Obrigações — a pagar vs a receber')} subtitle={tr('Fluxo mensal consolidado exceto salário CLT e imagem (aprox. BRL)')} tall>
              ...
              <>
                ...
                <ResponsiveContainer width="100%" height={340}>
                  <LineChart data={allDir} margin={{ top: 16, right: 56, bottom: 4, left: 8 }}>
                    ...
                    <Line type="monotone" dataKey="pagar" name={tr('A pagar')} stroke={C.pay} strokeWidth={1.5} strokeLinecap="round" dot={false} isAnimationActive={false} activeDot={{ r: 5, fill: C.pay, stroke: C.surface, strokeWidth: 2 }}>
                      <LabelList dataKey="pagar" content={<EndValueTag color={C.pay} total={allDir.length} />} />
                    </Line>
                    <Line type="monotone" dataKey="receber" name={tr('A receber')} stroke={C.recv} strokeWidth={1.5} strokeLinecap="round" dot={false} isAnimationActive={false} activeDot={{ r: 5, fill: C.recv, stroke: C.surface, strokeWidth: 2 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(460px, 100%), 1fr))', gap: 'var(--space-5)', marginTop: 'var(--space-5)' }}>
            ...
            <Panel eyebrow={tr('Clubes')} title={tr('A pagar vs a receber')} subtitle={tr('Transfer fee, sell-on, solidariedade — fluxo mensal (aprox. BRL)')}>
                ...
                <>
                  ...
                  <ResponsiveContainer width="100%" height={244}>
                    <LineChart data={clube} margin={{ top: 8, right: 56, bottom: 4, left: 8 }}>
                      ...
                      <YAxis tickLine={false} axisLine={false} width={72} tick={AXIS_TICK} tickFormatter={(v) => fmtCurrencyShort(v, 'BRL')} />
                      <Tooltip content={<BreakdownTip />} cursor={{ stroke: C.crosshair, strokeWidth: 1 }} />
                      <Line type="monotone" dataKey="pagar" name={tr('A pagar')} stroke={C.pay} strokeWidth={1.5} strokeLinecap="round" dot={false} isAnimationActive={false} activeDot={{ r: 4.5, fill: C.pay, stroke: C.surface, strokeWidth: 2 }}>
                        <LabelList dataKey="pagar" content={<EndValueTag color={C.pay} total={clube.length} />} />
                      </Line>
                      <Line type="monotone" dataKey="receber" name={tr('A receber')} stroke={C.recv} strokeWidth={1.5} strokeLinecap="round" dot={false} isAnimationActive={false} activeDot={{ r: 4.5, fill: C.recv, stroke: C.surface, strokeWidth: 2 }}>
            ...
            <Panel eyebrow={tr('Remuneração')} title={tr('Fluxo salarial (CLT + Imagem)')} subtitle={tr('Custo mensal consolidado dos elencos (aprox. BRL)')}>
              {salImg.length === 0 ? <Empty /> : (
                <ResponsiveContainer width="100%" height={244}>
                  <AreaChart data={salImg} margin={{ top: 16, right: 22, bottom: 4, left: 8 }}>
                    ...
                    <Area type="monotone" dataKey="total" name={tr('Custo mensal')} stroke={C.goldLine} strokeWidth={1.5} strokeLinecap="round" fill="url(#gSal)" dot={false} isAnimationActive={false} activeDot={{ r: 4.5, fill: C.goldLine, stroke: C.surface, strokeWidth: 2 }} />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </Panel>
            ...
            <Panel eyebrow={tr('Ranking')} title={tr('Top overdue por natureza')} subtitle={tr('Exposição vencida agrupada por tipo de obrigação')}>
              <HRank data={overdueByNat} color={C.late} empty="Nada em atraso" />
            </Panel>

            {/* Aging pie */}
            <Panel eyebrow={tr('Ranking')} title={tr('Aging da lista de vencidos')} subtitle={tr('Distribuição da exposição por faixa de atraso')}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 6, marginTop: 8 }}>
                  {aging.map(b => (
                    <span key={b.name} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-secondary)' }}>
                      ...
                      <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>{tr(b.name)}</span>
            ...
            <Panel eyebrow={tr('Ranking')} title={tr('Top overdue por clubes')} subtitle={tr('Clubes com maior exposição vencida')}>
              <HRank data={overdueByClube} color={C.late} empty="Nada em atraso com clubes" />
            </Panel>

            {/* Top overdue por agentes */}
            <Panel eyebrow={tr('Ranking')} title={tr('Top overdue por agentes')} subtitle={tr('Intermediários com maior exposição vencida')}>
              <HRank data={overdueByAgente} color={C.late} empty="Nada em atraso com agentes" />
            </Panel>

            {/* Top clubes mais pagos últimos 90 dias */}
            <Panel eyebrow={tr('Ranking')} title={tr('Top clubes mais pagos · 90 dias')} subtitle={tr('Volume liquidado nos últimos 90 dias')}>
            ...
            <Panel eyebrow={tr('Ranking')} title={tr('Top agentes mais pagos · 90 dias')} subtitle={tr('Volume liquidado nos últimos 90 dias')}>
          ...
          <div style={{ marginTop: 'var(--space-6)', display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
            {[
              ...
              <button key={x.to} onClick={() => navigate(x.to)} className="btn btn-outline">{tr(x.l)} <Icon name="chevronRight" size={16} /></button>
            ))}
          </div>
        </>
      )}
  ...
  return (
    <div className="panel-card" style={{ padding: tall ? 'var(--space-6)' : 'var(--gutter-card)' }}>
      <div style={{ marginBottom: 'var(--space-4)' }}>
        {eyebrow && <div className="eyebrow" style={{ marginBottom: 'var(--space-3)' }}>{tr(eyebrow)}</div>}
        <div style={{ fontFamily: font, fontSize: 'var(--text-subtitle-size)', fontWeight: 400, color: 'var(--ink-primary)', letterSpacing: '-0.01em' }}>{tr(title)}</div>
        {subtitle && <div style={{ fontFamily: font, fontSize: 'var(--text-body-sm-size)', color: 'var(--text-secondary)', marginTop: 4 }}>{tr(subtitle)}</div>}
      </div>
      {children}
    </div>
  )
...
function Empty({ msg = 'Sem dados.' }: { msg?: string }) {
  return <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: font, fontSize: 'var(--text-body-sm-size)', color: 'var(--text-secondary)' }}>{tr(msg)}</div>
}
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(160, data.length * 36 + 20)}>
      <BarChart layout="vertical" data={data} margin={{ top: 6, right: 96, bottom: 4, left: 8 }}>
        ...
        <YAxis type="category" dataKey="parte" width={132} tickFormatter={(v: string) => tr(v)} tickLine={false} axisLine={false} tick={{ fontSize: 12, fontFamily: font, fill: 'var(--text-secondary)' }} />
        <Tooltip content={<MoneyTip />} cursor={{ fill: 'var(--surface-sunken)' }} />
        <Bar dataKey="valor" name={tr('Valor')} fill={color} background={{ fill: 'var(--chart-track)', radius: 6 }} radius={[0, 6, 6, 0]} maxBarSize={20} barSize={16} isAnimationActive={false}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {rows.map(([nat, v]) => (
              <div key={nat} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: 12, color: 'var(--text-secondary)' }}>
                <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tr(nat)}</span>
...
function MoneyTip({ active, payload, label, leadKey }: any) {
  ...
  return (
    <div style={{ ...TIP, minWidth: 158 }}>
      <div style={{ color: 'var(--text-secondary)', marginBottom: 6, fontSize: 12 }}>{typeof label === 'string' ? tr(label) : label}</div>
      {lead && (
        <div style={{ marginBottom: rows.length ? 8 : 0, paddingBottom: rows.length ? 8 : 0, borderBottom: rows.length ? '1px solid var(--border-subtle)' : 'none' }}>
          <div className="eyebrow" style={{ marginBottom: 4 }}>{tr(lead.name)}</div>
...
function PieTip({ active, payload }: any) {
  ...
  return (
    <div style={{ ...TIP, minWidth: 220, maxWidth: 300 }}>
      <div className="eyebrow" style={{ marginBottom: 4 }}>{tr(p.name)}</div>
      ...
      {rows.length > 0 && (
        <>
          ...
          <div className="eyebrow" style={{ marginBottom: 4 }}>{tr('Por natureza')}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {rows.map(([nat, v]) => (
              <div key={nat} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: 12, color: 'var(--text-secondary)' }}>
                <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tr(nat)}</span>
