...
// Ordem de exibição por posição (de cima para baixo).
function positionRank(pos: string | null | undefined): number {
  ...
  if (p.includes('atacante') || p.includes('centroavante') || p.includes('ponta') || p.includes('atac')) return 5
  return 6
}
...
export default function PageAthletesList() {
  ...
  const filtered = useMemo(() => athletes.filter(a => {
    ...
    return r !== 0 ? r : (a.short_name || a.full_name).localeCompare(b.short_name || b.full_name)
              ...
              {filtered.map(a => {
                ...
                return (
                  <tr key={a.id} style={{ cursor: 'pointer' }}
                    ...
                    <td style={{ ...td, width: 130, fontSize: 12, color: a.position ? 'var(--ink-primary)' : 'var(--text-muted)' }}>
                      {a.position || '—'}
                    </td>
